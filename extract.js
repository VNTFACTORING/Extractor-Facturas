...
export default async function handler(req, res) {
  ...
  if (!fileData || !mimeType) return res.status(400).json({ error: 'Faltan parámetros: fileData o mimeType' });
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [content, {
            type: 'text',
            text: `Eres experto en documentos tributarios chilenos. Extrae los campos y responde SOLO con JSON válido sin markdown:
...
Montos como enteros sin puntos ni símbolos. Campos no visibles = null.`
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: err.error?.message || 'Error de API: ' + response.status });
    }
